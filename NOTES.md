...
**첫 만료가 승인 타임아웃보다 짧음** / **재처리 회차 밀림**

뒤의 둘은 Step 6에서 드러난 것이다. 재조회 간격을 300초에서 30초로 줄이면서 "거래 생성 후 첫 확인까지"와
"확인 사이의 간격"이 한 값으로 합쳐졌는데, 앞의 것은 승인 호출이 끝나기를 기다려야 하므로 성격이 다르다.
